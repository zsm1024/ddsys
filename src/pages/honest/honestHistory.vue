...
<section style="margin-bottom:0.1rem" id="writenote" >  
    <div ref="vies">
    <group label-width="4.5em" label-margin-right="1em" label-align="right" id="times">
      <x-input title="出差地点" name="place" v-model="times.travellocation" disabled></x-input>
      <x-input title="开始时间" name="starttime" v-model="times.begindate" disabled></x-input>
      <x-input title="结束时间" name="endtime" v-model="times.enddate" disabled></x-input>     
    </group>
     <group  label-width="4.5em" label-margin-right="2em" label-align="right">
        <x-textarea id="textar" name="description" :rows="2" v-model="times.tripcontent"  autosize disabled></x-textarea>
    </group>
    <cell title="记录日期" style="text-align:left;background:#fff;font-weight: bold;" class='write' >
        ...
        <datetime v-model="form.time2"  placeholder="请选择" clear-text="清除" @on-confirm='timechange' @on-clear="clearValue"></datetime>
    </cell>
    ...
    <group label-width="4.5em" label-margin-right="2em" label-align="right" class="foodtype">  
        <checklist title="早餐" :options="commonList" v-model="form.radioValue" disabled :max="1" style="display:flex"></checklist>
    </group>
    <group label-width="4.5em" label-margin-right="2em" label-align="right" class="foodtype foodtype1">
      <checklist title="午餐" :options="commonList2" v-model="form.radioValue1" disabled :max="1" style="display:flex"></checklist>   
       </group>
       <group label-width="4.5em" label-margin-right="2em" label-align="right" class="foodtype foodtype1">
      <checklist title="晚餐" :options="commonList2" v-model="form.radioValue2" disabled :max="1" style="display:flex"></checklist>
  </group>
  ...
  <group  label-width="4.5em" label-margin-right="2em" label-align="right">
    <x-input title="酒店名称" name="username" v-model="form.name" disabled placeholder="请输入酒店名称" ></x-input>
    <x-input title="电话号码" name="phone" v-model="form.phone" disabled type="number" placeholder="请输入电话号码" ></x-input>
    <x-input title=" 房间号" name="number" v-model="form.number" disabled placeholder="请输入房间号" ></x-input>
    <x-input title="  金额" name="money" v-model="form.money" disabled placeholder="请输入金额"></x-input>
    <group label-width="4.5em" label-align="right" class="foodtype" id="stay">
        <checklist title="住宿承担" :options="commonList3" disabled v-model="form.radioValue3" :max="1" style="display:flex"></checklist>
    </group>
  </group>
    ...
    <group  label-width="4.5em" label-margin-right="2em" label-align="right">
        <x-textarea v-model="form.receptionist" :rows="2" disabled autosize placeholder="请输入当天接洽人的职务、姓名，可输入多个接洽人" ></x-textarea>
    </group>
    ...
    <group  label-width="4.5em" label-margin-right="2em" label-align="right">
        <x-textarea v-model="form.daycontent" :rows="2"  disabled autosize  placeholder="请输入当天详细行程信息"></x-textarea>
    </group>
  </div>
...
<script>
import {gettraveldetail,getconcretedetailbytime } from "@/api/visit";
  ...
  methods: {
     changTitle(){
      dd.biz.navigation.setTitle({
          title : '历史记录',//控制标题文本，空字符串表示显示默认文本
      ...
      // if(this.$route.query.id){
        ...
        let paras={
         //userid:"091562293124062567",
          ...
          month:this.form.time2
        }
        getconcretedetailbytime(paras).then(res =>{
